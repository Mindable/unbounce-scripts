...
const app = Vue.createApp({
    data() {
        return {
            digital: true,
            productVariant: null,
            successRedirect: 'https://flux.astrologyanswers.com/?flux_action=1&flux_f=1061367355000511979&flux_ffn=1095133337314859374'
        }
    },
    methods: {
...
app.component('checkout-form', {
    ...
    `<form class="checkout-form" @submit.prevent="onSubmit">
        <div class="checkout-header">Contact Details</div>
        <div class="checkout-row">
            <label for="firstname">First Name*</label>
            <div class="checkout-col">
                <input class="checkout-input" type="text" id="firstname" required v-model="firstname">
            </div>
        </div>
        <div class="checkout-row">
          <label for="firstname">Last Name*</label>
          <div class="checkout-col">
            <input class="checkout-input" type="text" id="lastname" required v-model="lastname">
          </div>
        </div>
        <div class="checkout-row">
          <label for="firstname">Email*</label>
          <div class="checkout-col">
            <input class="checkout-input" type="email" id="email" required v-model="email">
          </div>
        </div>
        ...
        <div class="checkout-row">
          ...
          <div class="checkout-col">
            <input class="checkout-input" type="text" id="email" required v-model="email">
          </div>
        </div>
        <div class="checkout-row">
        ...
        <country-state type="billing"></country-state>
    ...
    methods: {
        onSubmit() {
            ...
            this.$emit('checkout-form-submitted', checkoutForm)
        },
        billingCountryStateSelection(billingCountryState){
            console.log('Billing Country State selected');
            console.log(billingCountryState);
            this.billingCountryState = billingCountryState;
        }
    }
});

app.component('country-state', {
    template: `<div class="checkout-row">
    <label for="firstname">Country*</label>
    <div class="checkout-col">
      <select v-model="countryId" @change="onCountrySelection()">
        <option v-for="country in countries" v-bind:value="country.countryCode">
          {{ country.countryName }}
        </option>
      </select>
    </div>
    </div>
    <div class="checkout-row" v-if="stateSelection">
    <label for="firstname">State*</label>
    <div class="checkout-col">
      <select v-model="stateId" @change="onStateSelection()">
        <option v-for="(stateValue, stateId) in states" :value="stateId">
          {{ stateValue }}
        </option> 
      </select>
    </div>
    ...
    methods: {
        onCountrySelection(){
            fetch(`https://aaproxyapis.astrologyanswerstest.com/countries/${this.countryId}/states`).then(response => {
                if (response.status === 200) {
                    response.json().then(data => {
                        console.log(data);
                        this.states = data;
                    ...
                    this.$emit(`${this.type}-country-state-selection`, CountryState);
                }
            });
        },
        onStateSelection(){
    ...
    computed: {
        ifCanada() {
            return this.countryId === 'CA';
        }
    }
});

const mountedApp = app.mount('#checkout-div');
// >-----End New Main Checkout
console.log('start');
(function () {
